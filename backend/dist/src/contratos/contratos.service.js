...
let ContratosService = class ContratosService {
    ...
    create(createContratoDto) {
        const { plano_manutencao } = createContratoDto, contratoData = __rest(createContratoDto, ["plano_manutencao"]);
    ...
    update(id, updateContratoDto) {
        const { plano_manutencao } = updateContratoDto, contratoData = __rest(updateContratoDto, ["plano_manutencao"]);
        if (typeof contratoData.data_inicio === 'string') {
            contratoData.data_inicio = new Date(contratoData.data_inicio);
        }
        if (typeof contratoData.data_fim === 'string') {
